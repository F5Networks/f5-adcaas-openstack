...
export class DeclarationController extends BaseController {
  ...
  private async loadService(service: Service): Promise<void> {
    ...
    if (service.profileHTTP) {
      let refs: {[k: string]: string} = {};
      let defs: {[k: string]: object} = {};
      if (await this.profileHTTPProfileRepository.exists(service.profileHTTP)) {
        let decl = await this.profileHTTPProfileRepository
          .findById(service.profileHTTP)
          .then(prof => prof.getAS3Declaration());
        defs[as3Name(service.profileHTTP)] = decl;
        refs = {use: as3Name(service.profileHTTP)};
      } else {
        refs = {bigip: `/Common/${service.profileHTTP}`};
      }

      let extName = as3ExtendedName('profileHTTP');
      service[extName] = {
        refs: refs,
        defs: defs,
      };
    ...
    if (service.profileHTTP2) {
      let refs: {[k: string]: string} = {};
      let defs: {[k: string]: object} = {};
      if (
        await this.profileHTTP2ProfileRepository.exists(service.profileHTTP2)
      ) {
        let decl = await this.profileHTTP2ProfileRepository
          .findById(service.profileHTTP2)
          .then(prof => prof.getAS3Declaration());
        defs[as3Name(service.profileHTTP2)] = decl;
        refs = {use: as3Name(service.profileHTTP2)};
      } else {
        refs = {bigip: `/Common/${service.profileHTTP2}`};
      }

      let extName = as3ExtendedName('profileHTTP2');
      service[extName] = {
        refs: refs,
        defs: defs,
      };
