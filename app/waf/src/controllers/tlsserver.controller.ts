...
export class TLSServerController extends BaseController {
  ...
  @post(prefix + '/tlsservers', {
    responses: {
      '200': Schema.response(TLSServer, 'Successfully create TLSServer resource'),
  ...
  async find(
    @param.query.object('filter', getFilterSchemaFor(TLSServer)) filter?: Filter,
  ...
  @get(prefix + '/tlsservers/{tlsserverId}', {
    responses: {
      responses: {
        '200': Schema.response(TLSServer, 'Successfully retrieve TLSServer resource'),
