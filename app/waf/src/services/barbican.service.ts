...
export abstract class BarbicanManager {
  ...
  abstract getSecret(userToken: string, bbcId: string): Promise<string>;
...
export class BarbicanManagerV1 extends BarbicanManager {
  async getSecret(userToken: string, bbcId: string): Promise<string> {
    ...
    const secretDetail = await this.barbicanService.getSecretDetail(
      userToken,
      url,
    );

    return JSON.parse(JSON.stringify(secretDetail))['body'][0];
  }
}
