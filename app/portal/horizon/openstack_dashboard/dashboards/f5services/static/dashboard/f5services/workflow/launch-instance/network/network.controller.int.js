...
(function () {
  ...
  /**
   * @ngdoc controller
   * @name LaunchInstanceIntNetworkController
   * @description
   * Controller for the Launch Instance - Network Step.
   */
  angular
    .module('horizon.dashboard.f5services.workflow.launch-instance')
    .controller('LaunchInstanceIntNetworkController', LaunchInstanceIntNetworkController);

    LaunchInstanceIntNetworkController.$inject = [
    '$scope',
    'horizon.framework.widgets.action-list.button-tooltip.row-warning.service',
    'launchInstanceModel'
  ];

  function LaunchInstanceIntNetworkController($scope, tooltipService, launchInstanceModel) {
    var ctrl = this;
    ...
    function toggleNetworksRequirement(newValue) {
      ...
      if (newValue.length > 0) {
        ctrl.tableDataMultiInt.minItems = 0;
      }
      // if no port is selected restore the validate-number-min value
      if (newValue.length === 0) {
        ctrl.tableDataMultiInt.minItems = 1;
      }
    }
