...
(function () {
  ...
  /**
   * @ngdoc controller
   * @name LaunchInstanceMgmtNetworkController
   * @description
   * Controller for the Launch Instance - Network Step.
   */
  angular
    .module('horizon.dashboard.f5services.workflow.launch-instance')
    .controller('LaunchInstanceMgmtNetworkController', LaunchInstanceMgmtNetworkController);

    LaunchInstanceMgmtNetworkController.$inject = [
    '$scope',
    'horizon.framework.widgets.action-list.button-tooltip.row-warning.service',
    'launchInstanceModel'
  ];

  function LaunchInstanceMgmtNetworkController($scope, tooltipService, launchInstanceModel) {
    var ctrl = this;
    ...
    ctrl.tableDataMultiMgmt = {
      available: $scope.model.networks,
      // allocated: $scope.model.newInstanceSpec.networks,
      allocated: $scope.model.newInstanceSpec.networksMap['mgmt'],
      displayedAvailable: [],
      displayedAllocated: [],
      minItems: 1
    };
    ...
    function toggleNetworksRequirement(newValue) {
      ...
      if (newValue.length > 0) {
        ctrl.tableDataMultiMgmt.minItems = 0;
      }
      // if no port is selected restore the validate-number-min value
      if (newValue.length === 0) {
        ctrl.tableDataMultiMgmt.minItems = 1;
      }
    }
