...
(function () {
  ...
  /**
   * @ngdoc controller
   * @name LaunchInstanceExtNetworkController
   * @description
   * Controller for the Launch Instance - Network Step.
   */
  angular
    .module('horizon.dashboard.f5services.workflow.launch-instance')
    .controller('LaunchInstanceExtNetworkController', LaunchInstanceExtNetworkController);

    LaunchInstanceExtNetworkController.$inject = [
    '$scope',
    'horizon.framework.widgets.action-list.button-tooltip.row-warning.service',
    'launchInstanceModel'
  ];

  function LaunchInstanceExtNetworkController($scope, tooltipService, launchInstanceModel) {
    var ctrl = this;
    ...
    function toggleNetworksRequirement(newValue) {
      ...
      if (newValue.length > 0) {
        ctrl.tableDataMultiExt.minItems = 0;
      }
      // if no port is selected restore the validate-number-min value
      if (newValue.length === 0) {
        ctrl.tableDataMultiExt.minItems = 1;
      }
    }
