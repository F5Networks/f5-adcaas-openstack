...
#             adc_tables.InstancesTable._meta.pagination_param, None)
...
class IndexView(tables.DataTableView):
    ...
    #                 search_opts=search_opts)
    #             instances.extend(tmp_instances)
    #             adc_list = api.f5wafaas.adc_list(self.request)
    #         except Exception:
    #             self._more = False
    #             exceptions.handle(self.request,
    #                               _('Unable to retrieve instances.'))
...
class RebuildView(forms.ModalFormView):
    form_class = adc_forms.RebuildInstanceForm
    template_name = 'f5services/f5adc/rebuild.html'
    success_url = reverse_lazy('horizon:f5services:f5adc:index')
...
class DecryptPasswordView(forms.ModalFormView):
    form_class = adc_forms.DecryptPasswordInstanceForm
    template_name = 'f5services/f5adc/decryptpassword.html'
    success_url = reverse_lazy('horizon:f5services:f5adc:index')
...
class DetailView(tabs.TabView):
    tab_group_class = adc_tabs.InstanceDetailTabs
    template_name = 'horizon/common/_detail.html'
    redirect_url = 'horizon:f5services:f5adc:index'
    ...
    def get_context_data(self, **kwargs):
        ...
        context["url"] = get_url_with_pagination(
            self.request,
            adc_tables.InstancesTable._meta.pagination_param,
            adc_tables.InstancesTable._meta.prev_pagination_param,
            self.redirect_url)
    ...
    def _get_actions(self, instance):
        table = adc_tables.InstancesTable(self.request)
    ...
    def get_data(self):
        ...
        choices = adc_tables.STATUS_DISPLAY_CHOICES
...
class AttachInterfaceView(forms.ModalFormView):
    form_class = adc_forms.AttachInterface
    template_name = 'f5services/f5adc/attach_interface.html'
    ...
    def get_initial(self):
        ...
        submit_url = "horizon:f5services:f5adc:attach_interface"
        self.submit_url = reverse(submit_url, kwargs=args)
        return args


class AttachVolumeView(forms.ModalFormView):
    form_class = adc_forms.AttachVolume
    template_name = 'f5services/f5adc/attach_volume.html'
    ...
    def get_initial(self):
        args = {'instance_id': self.kwargs['instance_id']}
        submit_url = "horizon:f5services:f5adc:attach_volume"
        self.submit_url = reverse(submit_url, kwargs=args)
        try:
...
class DetachVolumeView(forms.ModalFormView):
    form_class = adc_forms.DetachVolume
    template_name = 'f5services/f5adc/detach_volume.html'
    ...
    def get_initial(self):
        args = {'instance_id': self.kwargs['instance_id']}
        submit_url = "horizon:f5services:f5adc:detach_volume"
        self.submit_url = reverse(submit_url, kwargs=args)
        return {"instance_id": self.kwargs["instance_id"]}
...
class DetachInterfaceView(forms.ModalFormView):
    form_class = adc_forms.DetachInterface
    template_name = 'f5services/f5adc/detach_interface.html'
    ...
    def get_initial(self):
        args = {"instance_id": self.kwargs["instance_id"]}
        submit_url = "horizon:f5services:f5adc:detach_interface"
        self.submit_url = reverse(submit_url, kwargs=args)
        return args
