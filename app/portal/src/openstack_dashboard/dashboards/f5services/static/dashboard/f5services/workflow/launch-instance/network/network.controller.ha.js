...
(function () {
  ...
  /**
   * @ngdoc controller
   * @name LaunchInstanceHANetworkController
   * @description
  ...
  function LaunchInstanceHANetworkController($scope, tooltipService, launchInstanceModel) {
    var ctrl = this;
    ...
    function toggleNetworksRequirement(newValue) {
      ...
      if (newValue.length > 0) {
        ctrl.tableDataMultiHA.minItems = 0;
      }
      // if no port is selected restore the validate-number-min value
      if (newValue.length === 0) {
        ctrl.tableDataMultiHA.minItems = 1;
      }
    }
